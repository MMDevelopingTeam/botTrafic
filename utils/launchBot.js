...
// const launchBotVDos = async (proxy, name_model, username, password) => {
    const browser = await puppeteer.launch({
        args: [
            // "--proxy-server=185.249.1.151:8800 ",
            `--proxy-server=${proxy}`,
    ...
    try {
        ...
        await page.evaluate(() => {
            localStorage.setItem('defaultVideoQuality', '240p');
        });
        
        // await page.waitForTimeout(`${Math.floor((Math.random() * (15-10))+10)}000`);
        // await page.click('#close_entrance_terms')
        await page.waitForTimeout(`${Math.floor((Math.random() * (10-5))+5)}000`);
        console.log("Dentro de la pagina, PID:", browserPID);
        await page.waitForTimeout(`${Math.floor((Math.random() * (10-5))+5)}000`);
        await page.keyboard.press('h');
        await page.keyboard.press('o');
        await page.keyboard.press('l');
        ...
        try {
            ...
            console.log(`cuenta ${username} no logueada dentro del streaming`);
        }
...
module.exports = {launchBot, launchBotDos, launchBotVDos}